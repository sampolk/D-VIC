function [idx, iter, obj, H] = symnmf_cluster(X, k, options, Idx_NN)
%SYMNMF_CLUSTER
...
% try 
%     % attempts to compute the dense nxn matrix encoding pairwise distances
%     % between data points
% 
%     D = dist2(X, X);
...
%     clear D;
% 
%     % Enforces adjesency matrix (rather than Gaussian Kernel)
%     A(A>0) = 1;
%     
% catch
%     % Computes 
%     
%     if ~exist('Idx_NN', 'var')
%         % Need to compute A using knn search.
% 
%         Idx_NN = knnsearch(X,X, 'K', kk+1);
%         Idx_NN(:,1) =  [];
%     end
%     
%     % Preallocate memory for sparse matrix calculation
%     ind_row = repmat((1:size(X,1))', 1,kk);  % row indices for nearest neighbors.
%     ind_col = Idx_NN(:,1:kk);         % column indices for nearest neighbors.
% 
%     A = sparse(ind_row, ind_col, ones(size(X,1),kk)); % construct A    
%     
% end

% New code that uses previous nearest neighbor search 
if ~exist('Idx_NN', 'var')
    % Need to compute A using knn search.
...
% Code with sparsity incorporated
d = sum(A,2);
D12Inv = spdiags(1./sqrt(d),0,n,n);
A = D12Inv*((A +A')/2)*D12Inv;
...
if isinf(obj_best)
    % didn't converge. So, we assign random K-cluster clustering
    idx = randperm(length(A), k);
else
    idx = idx_best;
    iter = iter_best;
    obj = obj_best;
