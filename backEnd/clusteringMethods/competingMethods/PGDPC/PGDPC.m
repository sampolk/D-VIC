...
function [CL,Centers] = PGDPC(data,NN,K)
NN=NN+1;
close all
...
if d<=11
    [knn_pt,knn_dist] = knnsearch(data,data,'k',NN);
else
    dist = squareform(pdist(data));
    [knn_dist,knn_pt] = sort(dist,2);
end
...
for i=1:n 
    subL(i)=-1; 
end %% subL:sub label from subtrees (subclusters)
...
for i=1:n
    for j = 2:NN
        ...
        if subL(i)~=subL(jj) & edges(subL(i),subL(jj))> depth(i)+depth(jj)+1
            if find(knn_pt(jj,2:NN)==i);
                edges(subL(i),subL(jj)) = depth(i)+depth(jj)+1; 
                edges(subL(jj),subL(i)) = depth(i)+depth(jj)+1;
            end
        end
    end
end
...
phi_RNs = rho_RNs.*Delta_RNs';
[~,I_phi] = sort(phi_RNs,'descend');
for i=1:K
    NCLUST=NCLUST+1;
    CL_RNs(I_phi(i))=NCLUST;
    icl(NCLUST)=i;
end
%% Assignation
for i=1:NRNs
