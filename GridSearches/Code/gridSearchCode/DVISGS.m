    ...
    for i = [1,3]
        for j = [6:13, 15:19]
            for k = 1:numReplicates
                ...
                disp(['DVIS: '])
                disp([i/length(NNs), j/length(prctiles), k/numReplicates, bestPerf])

            end
            currentPerf = mean(squeeze(OAs(i,j,:)));
            if currentPerf >= bestPerf
                bestPerf = currentPerf;
                save(strcat('DVISHP', datasets{dataIdx}), 'Hyperparameters')
            end
        end 
